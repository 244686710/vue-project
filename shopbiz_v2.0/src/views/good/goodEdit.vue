            ...
            <div class="basic" v-show="model.step == 1">
                ...
                <ul>
                    ...
                    <li class="item clearfix price">
                        <div class="f_left">
                            ...
                            <div class="f_left border">
                                <i>￥</i>
                                <input type="text" placeholder="0.00" @change="writeWeight('price')" v-model="model.price" @keyup="model.price =  model.price && model.price.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')" @input="isEmpty(model.price, 'price')" maxlength="6">
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="f_left border">
                                <i>￥</i>
                                <input type="text" placeholder="0.00" v-model="model.originalPrice" @change="writeWeight('originalPrice')" maxlength="6" @keyup="model.originalPrice =  model.originalPrice && model.originalPrice.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')">
                            </div>
                        </div>
                  ...
                  <i class="close icon-close f_right" @click="LevelShow = false"></i>
                    ...
                    <li class="item upload clearfix">
                        <label for="" class="item_name">*商品图片：</label> 建议图片尺寸：800*800像素，大小不超过3M，通过拖动图片调整图片顺序
                ...
                <ul>
                    ...
                    <li class="item" v-show="model.attrs.length == 0">
                        ...
                        <input type="text" v-model="model.stock" @keyup="model.stock = model.stock.replace(/[^0-9]/g,'')" maxlength="6">
                    ...
                    <li class="item" v-show="model.goodsType == 1">
                ...
                <ul>
                    <li class="item clearfix delivery" v-show="model.goodsType == 1">
                        <label class="f_left item_name" for="">运费设置： </label>
                        <div class="clearfix price overflwo">
                            <a href="javascript:;" class="f_left icon_radio icon-radio-blue-default" :class="{'icon-radio-blue-checked': model.deliveryFeeType == 1}" @click="deliveryType(1)"></a>
                            <div class="f_left">统一邮费&nbsp;&nbsp;</div>
                            <div class="border f_left">
                                <i class="">￥</i>
                                <input type="text" class="bg_white" v-model="model.deliveryFee" placeholder="0.00" @keyup="model.deliveryFee = model.deliveryFee.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')" :disabled="model.goodsType == 2">
                            </div>
                        </div>
                        <div class="price last" v-if="deliverySwitchType == 1" v-show="model.goodsType !== 2">
                            <a href="javascript:;" class="f_left icon_radio icon-radio-blue-default" :class="{'icon-radio-blue-checked': model.deliveryFeeType == 2}" @click="deliveryType(2)"></a>
                  ...
                  <i class="close icon-close f_right" @click="shopCatShow = false"></i>
                        ...
                        <a href="javascript:;" class="handle" @click="toCreateShopCar()">新建分类</a>
        ...
        data() {
            return {
                config: {
                    ...
                    elementPathEnabled: false, // 关闭元素路径统计
        ...
        methods: {
            ...
            writeWeight(key) {
                ...
                if(this.model[key]) {
                    ...
                    if(val >= 10000000) {
                        this.$utils.layerMsg.warning('商品价格不可以大于10000000')
                        this.model[key] = ''
                        this.model = Object.assign({}, this.model)
                        return '';

                    }
                    if(Number(this.model[key])) {
                        this.model[key] = this.model[key];
                    } else if(Number(this.model[key]) !== 0) {
                        this.model[key] = ''
                        this.model = Object.assign({}, this.model)
                        return false;
                    }
                    ...
                    if(reg.test(val)) {
                        val=val.toString()
                        if((val.indexOf('.') == -1 && val.length <= 6)||val>0.01) {
                            this.model[key] = parseFloat(val).toFixed(2)
                        } else {
                            this.model[key] = 0.01
                        }
            ...
            nextStep() {
                ...
                if(this.model.attrs.length == 0) {
                    ...
                    that.model.skus.map(item => {
                        ...
                        if(item.spec.stock && item.spec.stock < 0) {
                            that.$utils.layerMsg.error('库存必须大于等于0')
                            isSpec = false
                        }
                        if(item.spec.stock && item.spec.stock) {
                ...
                if((!this.model.weight || this.model.weight < 0) && this.model.goodsType == 1) {
